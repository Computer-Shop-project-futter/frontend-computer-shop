...
class AuthNotifier extends StateNotifier<AuthState> {
  ...
  DateTime? _registrationLockUntil;
  ...
  Future<bool> verifySignupCode(String email, String code) async {
    state = const AuthState.loading();
    try {
      final result = await _repository.verifySignupCode(email, code);
      if (result.token != null) {
        await _saveToken(result.token!);
      }
      state = AuthState.authenticated(result.user);
  ...
  Future<void> register(
    ...
    // Enforce client-side cooldown when backend rate-limits email sending
    if (_registrationLockUntil != null && DateTime.now().isBefore(_registrationLockUntil!)) {
      final waitSec = _registrationLockUntil!.difference(DateTime.now()).inSeconds;
      state = AuthState.error('Too many attempts. Please wait ${waitSec}s and try again.');
      return;
    }
    ...
    try {
      ...
      final message = error is Exception ? error.toString().replaceFirst('Exception: ', '') : error.toString();
      // If backend reports rate limiting, set a short client-side lockout
      final lower = message.toLowerCase();
      if (lower.contains('rate limit') || lower.contains('too many') || lower.contains('over_email_send_rate_limit')) {
        _registrationLockUntil = DateTime.now().add(const Duration(minutes: 2));
      }
      state = AuthState.error(message);
    }
  }
