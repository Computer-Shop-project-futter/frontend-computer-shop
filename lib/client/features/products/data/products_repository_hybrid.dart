...
class ProductsRepositoryHybrid {
  ...
  Future<PaginatedProducts> getProducts(
    ...
    // On web, local SQLite is not available — use Supabase directly
    if (kIsWeb) {
      return await _api.getProducts(filters, page: page);
    }
  ...
  Future<ProductDetailPayload> getProductById(String productId) async {
    // On web, avoid local SQLite and use Supabase
    if (kIsWeb) {
  ...
  Future<void> _syncPendingOperations() async {
    // Sync any pending offline operations when back online
  ...
  Future<PaginatedProducts> getFeaturedProducts({int page = 1}) async {
    if (kIsWeb) return await _api.getFeaturedProducts(page: page);
  ...
  Future<PaginatedProducts> getDeals({int page = 1}) async {
    if (kIsWeb) return await _api.getDealProducts(page: page);
