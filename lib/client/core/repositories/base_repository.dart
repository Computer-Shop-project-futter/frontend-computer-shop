...
abstract class BaseRepository {
  ...
  Future<void> addToSyncQueue({
    ...
    final db = await database;
    final resolvedEntityId = entityId ?? _resolveEntityId(tableName, data);

    // Check if already in queue (prevent duplicate pending operations)
    final existing = await db.query(
      'sync_queue',
      where: 'entity_id = ? AND operation = ? AND synced = 0',
      whereArgs: [resolvedEntityId, operation],
    );
    ...
    await db.insert(
      'sync_queue',
      {
        'operation_id': '${DateTime.now().millisecondsSinceEpoch}_$resolvedEntityId',
        'operation': operation,
  ...
  String _resolveEntityId(String tableName, Map<String, dynamic> data) {
    return data['item_id']?.toString()
        ?? data['cart_item_id']?.toString()
        ?? data['build_id']?.toString()
