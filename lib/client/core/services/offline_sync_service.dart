...
import '../../features/favorites/data/wishlist_repository_supabase.dart';
import '../../features/cart/data/cart_repository_supabase.dart';
import '../../features/pc_builder/data/builder_repository_supabase.dart';
import '../../features/repair/data/repair_repository_supabase.dart';
import '../../features/favorites/domain/wishlist_model.dart';
import '../../features/cart/domain/cart_model.dart';
import '../../features/pc_builder/domain/builder_model.dart';
import '../../features/repair/domain/repair_model.dart';
...
class OfflineSyncService {
  ...
  Future<bool> _syncWishlistOperation(String operation, Map<String, dynamic> data) async {
    final repository = WishlistRepositorySupabase();
    switch (operation) {
      case 'INSERT':
        final item = WishlistItem.fromJson({
          ...
          'type': data['type'],
          'addedAt': data['added_at'] ?? DateTime.now().millisecondsSinceEpoch,
          'metadata': data['metadata'],
        });
        await repository.addToWishlist(item);
        return true;
      case 'DELETE':
        final itemId = data['item_id']?.toString() ?? data['id']?.toString();
        if (itemId == null) return false;
        await repository.removeFromWishlist(itemId);
        return true;
      case 'CLEAR':
  ...
  Future<bool> _syncCartOperation(String operation, Map<String, dynamic> data) async {
    final repository = CartRepositorySupabase();
    switch (operation) {
      case 'INSERT':
        final item = CartItem.fromDbJson({
          'cart_item_id': data['cart_item_id'] ?? data['id'],
          ...
          'added_at': data['added_at'] ?? DateTime.now().millisecondsSinceEpoch,
        });
        await repository.addToCart(item);
        return true;
      case 'UPDATE':
        final cartItemId = data['cart_item_id']?.toString() ?? data['id']?.toString();
  ...
  Future<bool> _syncBuildOperation(String operation, Map<String, dynamic> data) async {
    final repository = BuilderRepositorySupabase();
    switch (operation) {
      case 'INSERT':
      case 'UPDATE':
        final build = BuildConfiguration(
          id: data['build_id']?.toString() ?? '',
          name: data['name']?.toString() ?? 'Build',
          createdAt: DateTime.fromMillisecondsSinceEpoch(data['created_at'] as int? ?? DateTime.now().millisecondsSinceEpoch),
          cpu: null,
          gpu: null,
          motherboard: null,
          ram: null,
        ...
        await repository.saveBuild(build, userId: data['user_id']?.toString());
        return true;
      case 'DELETE':
        final buildId = data['build_id']?.toString();
        if (buildId == null) return false;
        await repository.deleteBuild(buildId);
        return true;
      default:
        return false;
    }
  ...
  Future<bool> _syncRepairOperation(String operation, Map<String, dynamic> data) async {
    final repository = RepairRepositorySupabase();
    switch (operation) {
      case 'INSERT':
        await repository.submitRepairRequest(
          repairId: data['repair_id']?.toString() ?? '',
          orderNumber: data['order_number']?.toString() ?? '',
          deviceType: DeviceType.values.firstWhere(
            (type) => type.toString().split('.').last == data['device_type'],
            orElse: () => DeviceType.laptop,
          ),
          deviceModel: data['device_model']?.toString() ?? '',
          issues: List<String>.from(data['issues'] ?? []),
          description: data['description']?.toString() ?? '',
          submittedAt: DateTime.fromMillisecondsSinceEpoch(data['submitted_at'] as int? ?? DateTime.now().millisecondsSinceEpoch),
          ...
          scheduledDate: data['scheduled_date'] != null
              ? DateTime.fromMillisecondsSinceEpoch(data['scheduled_date'] as int)
              : null,
          technicianName: data['technician_name']?.toString(),
        );
        return true;
      case 'UPDATE':
        final repairId = data['repair_id']?.toString();
        if (repairId == null) return false;
        await repository.cancelRepair(repairId);
        return true;
      default:
        return false;
    }
